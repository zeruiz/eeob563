...
The principle of the maximum likelihood approach to tree construction is to estimate parameters based on different assumptions about the base frequencies and the substitution matrix such that the found parameters give the maximum likelihood value on the given multiple sequence alignment. To verify the accuracy of the tree, bootstrap analysis would resample columns of characters out of the result and rebuild the tree for dozens of times, and for each time it would test if the same nodes were recovered. The final proportions of recovery were mapped on the best scoring ML tree in figure 4.

ML tree inferece here was under the GTR+GAMMA (general time reversible model of nucleotide
substitution with a $\Gamma$ model of rate heterogeneity) which meant the base frequencies were different and the substituion matrix is symmetrical; the rate variation among sites in a sequence was modeled by Gamma distribution. FBP (binary presence/absence of bipartitions from replicate trees in the best-scoring ML tree) and TBE (the minimum number of taxa that have to be transfered or removed to make those branches identical) were found similar for the bootstrap support analysis, and the grouping of best-scoring ML tree was quite similar to the previous two.
...
The Bayesian approach is also a model-based parametric approach, except that the point estimates in the maximum likelihood approach become the parameter estimation within the posterior distribution here. The analysis was performed on the unpartitioned alignments using the same GTR+GAMMA model. Again, the consensus tree from the later cycles of MCMC was still similar to the previous ones.
...
From the rooted tree, one topological hypothesis between ARMS2 from human and that from gorilla was conducted to see whether they were more closely related to each other than the others or not. Some experimental data were recorded and shown in table 1. The average standard deviation that approached to 0 along the run was the signal of convergence of MCMC and from the result we could assume the convergence was achieved for both of the analysis.
...
knitr::include_graphics("arms2.out.con.tre.hp.jpg")
``` 
\begin{table}[ht]
